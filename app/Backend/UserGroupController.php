...
class UserGroupController {
    ...
    public function index() {
        ...
        if ($redirect = Security::check('user_group.read'))
            return $redirect;
    ...
    public function _new() {
        if ($redirect = Security::check('user_group.create'))
            return $redirect;
        $vars = ['method' => 'add'];
    ...
    public function edit() {
        if ($redirect = Security::check('user_group.update'))
            return $redirect;
        $vars['method'] = 'edit';
        if (Request::isPost()) {
            $group_name = post('group_name');
    ...
    public function delete() {
        if ($redirect = Security::check('user_group.delete'))
            return $redirect;
        $id = getInt("id");
    ...
    public function permission() {
        if ($redirect = Security::check('user_group.permission_setting'))
            return $redirect;
        $vars = [];
